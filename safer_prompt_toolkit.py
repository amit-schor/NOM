...
def prompt(message, max_failcase_completion_lines=3, max_chars_in_completion_line=150, force_fail_safe=False,
           **prompt_toolkit_kwargs):
    """
    :param force_fail_safe: fails the use of prompt_toolkit if True, so only safer_prompt_toolkit functions will be called
    ...
    """
    try:
        assert not force_fail_safe
        # try using prompt toolkit
        return prompt_toolkit.prompt(message, **prompt_toolkit_kwargs)
    except (NoConsoleScreenBufferError, AssertionError):
        # if doesn't work than try using the simplest form of input possible
        return simplest_prompt(message, max_failcase_completion_lines=max_failcase_completion_lines,
                               max_chars_in_completion_line=max_chars_in_completion_line, **prompt_toolkit_kwargs)


def simplest_prompt(message, max_failcase_completion_lines=3, max_chars_in_completion_line=150,
                    **prompt_toolkit_kwargs):
    validator: validation.Validator = prompt_toolkit_kwargs[
        "validator"] if "validator" in prompt_toolkit_kwargs else None
    completer: completion.Completer = prompt_toolkit_kwargs[
        "completer"] if "completer" in prompt_toolkit_kwargs else None
